...
aws.get("/", async (c) => {
    ...
    return c.json(
      {
        ...
        examples: {
          ...
          byCityGeoJSON: "/aws?city=cilacap&format=geojson",
          byRadius: "/aws?lat=-7.5&lon=110.5&radius=50",
          byRadiusGeoJSON: "/aws?lat=-7.5&lon=110.5&radius=50&format=geojson",
  ...
  try {
    ...
    if (provinceParam) {
      ...
      results = await fetcher.fetchDataByProvince(
        provinceCodes,
        type,
        cityFilter,
        excludeCities,
    ...
    if (formatParam === "geojson") {
      // Transform results to AWSStation format for GeoJSON
      const stations = successful.map((result: any) => ({
        id_station: result.stationId,
        name_station: result.stationName || "N/A",
        nama_kota: result.city || "N/A",
        lat: result.lat || "0",
        lng: result.lng || "0",
        icon: result.data?.icon || "",
        type: result.type || "aws",
        diff_day: result.data?.diff_day || "0",
        diff_hour: result.data?.diff_hour || "0",
        diff_minute: result.data?.diff_minute || "0",
        tanggal: result.data?.tanggal || new Date().toISOString(),
        kode_provinsi: result.provinceCode || "",
        nama_provinsi: result.province || "",
        klasifikasi: result.data?.klasifikasi || null,
      ...
      const geojson = awsToGeoJSON(stations, true);

      console.log("📤 Sending GeoJSON response...");
      console.log("Response size:", JSON.stringify(geojson).length, "bytes");
